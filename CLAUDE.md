...
**Anonymous background agents (same CLI, unnamed spawns)**: `Agent(...)` without a `name` takes a different path — result "Async agent launched successfully. agentId: \<hex>" (no team anywhere), transcript + sidecar under `<projectDir>/<leadSessionId>/subagents/` (sidecar carries `agentType`/`description`/`toolUseId`), completion via `queue-operation`. These are promoted to named characters by `scanForBackgroundAgentFiles`: the sidecar `toolUseId` must match a LIVE entry in the lead's `backgroundAgentToolIds` (the anti-spurious gate; no teamName involved). The promoted agent has `leadAgentId` + `spawnToolUseId` but NO `teamName` (config polling must not touch it); it's removed on the completion queue-operation or lead sessionEnd. The turn-end background-tool re-send must include `toolName` + `runInBackground` (or the webview can't recreate the Subtask after `agentToolsClear`) and must skip promoted tools (or a ghost Subtask appears next to the real character).
