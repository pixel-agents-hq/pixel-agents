...
**Lounge spots (custom addition, not upstream)**: Idle (non-working) agents no longer periodically sit back at their desk miming typing — they path to and stay at a break-area chair instead, only returning to their desk when actually given work again. Any chair/sofa/bench whose seat tile falls inside an Area labeled exactly `"Lounge"` (`LOUNGE_AREA_LABEL` in `webview-ui/src/constants.ts`) is carved out of the normal work-seat pool by `partitionSeatsByLoungeArea()` (`office/layout/layoutSerializer.ts`) into `OfficeState.loungeSpots` — same underlying `Seat` shape, just a second pool. Identical furniture placed _outside_ the Lounge area (e.g. a bench under a desk) still works as a normal seat; it's about placement, not furniture type. FSM changes live in `characters.ts`'s `IDLE` case (`Character.loungeId`, `claimFreeLoungeSpot`/`releaseLoungeSpot`) — claiming a spot and pathing to it are split across two ticks (claim now, path next tick) because the tile is only unblocked for pathfinding once `OfficeState.withOwnSeatUnblocked` sees the character's `loungeId` set. **To change**: add/move the "Lounge" Area over whatever chairs/sofas should be chill-only (via the in-app Area Paint tool, or by editing `areas`/`areaTiles` directly in the layout JSON) — no code change needed to relocate or resize the break room.
...
**Loading**: `esbuild.js` copies `webview-ui/public/assets/` → `dist/assets/`. Loader checks bundled path first, falls back to workspace root. PNG → pngjs → SpriteData (2D hex array, alpha≥2 = visible, `#RRGGBBAA` for semi-transparent). `loadDefaultLayout()` scans for `assets/default-layout-{N}.json` and picks the highest `N` (falls back to unversioned `default-layout.json`). Current default is `default-layout-2.json` (31×22, 8 desks, two sofa clusters in a `"Lounge"`-tagged Area) — bump to `default-layout-3.json` with an incremented `layoutRevision` for any future default-office redesign; don't edit `-2` in place, since anyone already on revision 2 wouldn't get re-reset. Generated with a throwaway Node script (grid + furniture placed by formula, validated for footprint collisions before writing) rather than hand-edited JSON — worth doing the same for any large layout change.
...
- **Standalone CLI didn't honor `layoutRevision` resets (fixed)**: `loadLayout()` (`server/src/layoutPersistence.ts`) compares the bundled default's `layoutRevision` against the saved file's and resets when the bundled one is newer — but it was only ever called from the VS Code adapter (`adapters/vscode/agentManager.ts`). The standalone path's `webviewReady` handler in `clientMessageHandler.ts` called the plain `readLayoutFromFile()` instead, so a saved `~/.pixel-agents/layout.json` was returned unconditionally no matter how the bundled default's revision changed. Fixed by switching that handler to `loadLayout(cache?.defaultLayout)` too, forwarding `wasReset` in the `layoutLoaded` message (the field was already in the AsyncAPI schema, just unused on this path). If a future default-layout bump silently doesn't show up for standalone users, check this code path first.
- **Rebuilding after a source change doesn't always show up**: the standalone server has a "reuse existing instance" feature — `npx pixel-agents` checks `~/.pixel-agents/server.json` and reconnects to an already-running process (by PID) instead of starting a new one. If you edit source and rebuild while an old server process is still alive, relaunching just reconnects you to the STALE process running old code. Kill it first (check the PID in `server.json` against `tasklist`/`ps`) before relaunching, or just always kill any lingering `node` process for this app before testing a rebuild.
