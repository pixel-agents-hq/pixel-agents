...
export function DebugView({
  ...
  const [diagnostics, setDiagnostics] = useState<
    Record<number, AgentDiagnostics>
  >({});
        ...
        {diag && (
          <div className="mt-6 py-4 px-6 text-xs opacity-70 flex flex-col gap-2 border-t border-white/8">
            <span>
              <span
                className={
                  diag.jsonlExists ? "text-status-success" : "text-status-error"
                }
              >
            ...
            <span className="opacity-60 text-2xs break-all">
              {diag.jsonlFile}
            </span>
            ...
            {diag.jsonlExists &&
              diag.fileSize > 0 &&
              diag.linesProcessed === 0 && (
                <span className="text-2xs text-status-permission">
                  File has data ({diag.fileSize} bytes) but 0 lines parsed.
                  Possible format issue.
                </span>
              )}
          </div>
        )}
