...
describe('terminal token extraction', () => {
  ...
  it('extracts against a caller-supplied protocol', () => {
    // Each socket only accepts its own protocol name in the first slot.
    const other = 'pixel-agents.other.v1';
    expect(extractTokenFromProtocolHeader(`${other}, tok-9`, other)).toBe('tok-9');
    expect(extractTokenFromProtocolHeader(`${TERMINAL_WS_PROTOCOL}, tok-9`, other)).toBeNull();
  });
});
