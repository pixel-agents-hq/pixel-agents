...
describe("HookEventHandler", () => {
  ...
  it("PermissionRequest notifies sub-agents", () => {
    const agent = createTestAgent({ id: 1 });
    agent.activeSubagentToolNames.set(
      "tool-parent",
      new Map([["sub-1", "Read"]]),
    );
    agents.set(1, agent);
    ...
    const subMsg = mockWebview.messages.find(
      (m) => m.type === "subagentToolPermission",
    );
  ...
  it("Stop clears foreground tools but preserves background agents", () => {
    ...
    const clearMsg = mockWebview.messages.find(
      (m) => m.type === "agentToolsClear",
    );
  ...
  it("PreToolUse sends agentToolStart with formatted status", () => {
    ...
    const toolMsg = mockWebview.messages.find(
      (m) => m.type === "agentToolStart",
    );
  ...
  it("PostToolUse sends agentToolDone and clears currentHookToolId", () => {
    ...
    const doneMsg = mockWebview.messages.find(
      (m) => m.type === "agentToolDone",
    );
  ...
  it("SessionStart(source=resume) calls onSessionResume", () => {
    ...
    expect(onSessionResume).toHaveBeenCalledWith(
      "/projects/test/resume-sess.jsonl",
    );
  });
  ...
  describe.each([
    {
      label: "run_in_background=true",
      toolInput: { run_in_background: true },
      expected: true,
    },
    {
      label: "run_in_background=false",
  ...
  describe.each([
    ...
    {
      label:
        "basic subagent from hook before JSONL (no activeToolNames parent)",
      spawn: false,
      teamName: undefined,
      seedActiveTool: false,
      expectTeammateDetected: false,
    ...
    "SubagentStart: $label",
    ({ spawn, teamName, seedActiveTool, expectTeammateDetected }) => {
      it(`onTeammateDetected called=${expectTeammateDetected}`, () => {
        ...
        handler.handleEvent("claude", {
          ...
          session_id: "sess-1",
          agent_type: expectTeammateDetected
            ? "web-researcher"
            : "general-purpose",
        });

        if (expectTeammateDetected) {
          expect(onTeammateDetected).toHaveBeenCalledWith(
            1,
            "sess-1",
            "web-researcher",
          );
          ...
          expect(
            mockWebview.messages.find((m) => m.type === "subagentToolStart"),
          ).toBeUndefined();
        } else {
          expect(onTeammateDetected).not.toHaveBeenCalled();
          const msg = mockWebview.messages.find(
            (m) => m.type === "subagentToolStart",
          );
          if (seedActiveTool) {
