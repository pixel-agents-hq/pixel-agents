...
- `e2e/claude/hooks-on/basic.spec.ts:37` — internal terminal spawns agent and Task subagent appears then despawns (Hooks ON / spawn paths)
- `e2e/claude/hooks-on/basic.spec.ts:123` — external Claude session adopted via hook confirmation lifecycle (Hooks ON / spawn paths)
- `e2e/claude/hooks-on/basic.spec.ts:214` — agentDeselected clears the office label panel opened by agentSelected (Hooks ON / spawn paths)
