...
118 tests total. Generated by `scripts/generate-e2e-inventory.mjs`. Re-run after adding or removing tests.

### `@area:spawn` (5 tests)

- `e2e/claude/hooks-off/launch-drawer.spec.ts:40` — hovering + Agent opens a drawer with a badged, read-only row per workspace folder (Launch drawer (multi-root))
- `e2e/claude/hooks-off/launch-drawer.spec.ts:63` — tapping a drawer row launches an agent labeled with that Directory (Launch drawer (multi-root))
- `e2e/claude/hooks-off/launch-drawer.spec.ts:80` — a Directory defined in the office joins the drawer, launches, and survives a panel reload (Launch drawer (multi-root))
...
- `e2e/claude/hooks-on/basic.spec.ts:125` — external Claude session adopted via hook confirmation lifecycle (Hooks ON / spawn paths)
...
- `e2e/standalone/launch-drawer.spec.ts:201` — an invalid path is refused inline and nothing is persisted (Standalone / Directory validation)
- `e2e/standalone/launch-drawer.spec.ts:250` — a past session offers its working directory as a tap that fills the path (Standalone / Directory suggestions)
- `e2e/standalone/launch-drawer.spec.ts:303` — the skip-permissions setting lands on disk and survives a reload (Standalone / permission posture)
...
- `e2e/standalone/terminal.spec.ts:26` — browser-launched agent gets a PTY terminal, hooks route to its character, close cleans up (Standalone / terminal)
- `e2e/standalone/terminal.spec.ts:86` — --no-terminal keeps the office watch-only: launch disabled, hook-driven agents still render (Standalone / terminal disabled)
...
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:84` — painting an area labels tiles in the layout (Areas (multi-root))
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:112` — areas can be added and removed (Areas (multi-root))
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:145` — a folder can be mapped to an area and the mapping persists (Areas (multi-root))
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:180` — an agent for the MAPPED folder takes a seat inside its area (Areas (multi-root) › seat preference (alpha → Engineering))
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:211` — an agent for an UNMAPPED folder is not forced into the area (Areas (multi-root) › seat preference (alpha → Engineering))
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:255` — a Directory assigned an Area in its modal seats the agent it launches inside that Area (Areas (multi-root) › Area assignment from the Directory modal)
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:308` — renaming a Directory carries its Area mapping to the new name (Areas (multi-root) › Area assignment from the Directory modal › rename)
...
- `e2e/claude/hooks-off/areas.spec.ts:72` — the host-contributed workspace folder makes the Areas tool reachable (Areas (single-folder))
- `e2e/claude/hooks-off/areas.spec.ts:92` — the Areas tool button is visible with a seeded areas layout (Areas (single-folder) › seeded areas layout (positive gate))
...
### `@area:editor` (14 tests)

- `e2e/claude/hooks-off/editor.spec.ts:85` — erasing a tile deletes the furniture it passes through (Layout editor — erase)
- `e2e/claude/hooks-off/editor.spec.ts:108` — erasing away from an item leaves it in place (Layout editor — erase)
- `e2e/claude/hooks-off/editor.spec.ts:130` — one undo restores both the tiles and the furniture an erase stroke removed (Layout editor — erase)
- `e2e/claude/hooks-off/editor.spec.ts:168` — a right-drag erase stroke is a single undo entry (Layout editor — erase)
- `e2e/claude/hooks-off/editor.spec.ts:207` — a floor paint stroke is a single undo entry (Layout editor — stroke undo)
- `e2e/claude/hooks-off/editor.spec.ts:245` — a wall paint stroke is a single undo entry (Layout editor — stroke undo)
- `e2e/claude/hooks-off/editor.spec.ts:281` — separate strokes stay separate undo entries (Layout editor — stroke undo)
- `e2e/claude/hooks-off/editor.spec.ts:366` — dragging a desk moves the item standing on it (Layout editor — desks carry what stands on them)
- `e2e/claude/hooks-off/editor.spec.ts:389` — a drag that would push the item on the desk off the grid is refused (Layout editor — desks carry what stands on them)
- `e2e/claude/hooks-off/editor.spec.ts:409` — rotating a desk turns the item standing on it (Layout editor — desks carry what stands on them)
- `e2e/claude/hooks-off/editor.spec.ts:435` — alt-dragging a desk copies it with the item standing on it (Layout editor — desks carry what stands on them)
- `e2e/claude/hooks-off/editor.spec.ts:465` — an alt-drag copy that overlaps the original is refused (Layout editor — desks carry what stands on them)
- `e2e/claude/hooks-off/editor.spec.ts:495` — one undo removes the whole copied group (Layout editor — desks carry what stands on them)
- `e2e/claude/hooks-off/editor.spec.ts:513` — selecting a placed item closes the open tool panel, and R rotates it (Layout editor — desks carry what stands on them)
