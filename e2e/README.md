...
96 tests total. Generated by `scripts/generate-e2e-inventory.mjs`. Re-run after adding or removing tests.
...
- `e2e/claude/hooks-on/lifecycle.spec.ts:1429` — pixel-agents hook is installed in settings.json on extension startup (Hooks ON / lifecycle)
- `e2e/claude/hooks-on/lifecycle.spec.ts:1451` — hook install and uninstall round-trip via the Settings toggle (Hooks ON / lifecycle)
- `e2e/claude/hooks-on/lifecycle.spec.ts:1505` — permission bubble auto-clears when a fresh PreToolUse arrives (Hooks ON / lifecycle)
- `e2e/claude/hooks-on/lifecycle.spec.ts:1579` — settings toggles persist across a webview reload (Hooks ON / lifecycle)
- `e2e/claude/hooks-on/lifecycle.spec.ts:1624` — Settings reports the resolved Claude config directory (Hooks ON / lifecycle)
- `e2e/claude/hooks-on/lifecycle.spec.ts:1654` — layout editor enter paint save persist and exit round-trip (Hooks ON / lifecycle)
- `e2e/claude/hooks-on/lifecycle.spec.ts:1735` — hook uninstall preserves a pre-existing third-party hook entry (Hooks ON / lifecycle)
