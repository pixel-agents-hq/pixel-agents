...
91 tests total. Generated by `scripts/generate-e2e-inventory.mjs`. Re-run after adding or removing tests.
...
- `e2e/claude/hooks-on/consent.spec.ts:137` — fresh install: the Intro pages to the disclosure and Install writes the hooks (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:229` — Not Now writes nothing, continues the tour, and leaves consent ungranted (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:256` — Don't Ask Again writes nothing and persists hooks off (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:281` — the close x aborts the tour and writes nothing, exactly like Not Now (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:310` — Back from the closing step lets Don't Ask Again undo a landed install (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:346` — Back after Don't Ask Again lets Not Now bring the ask back (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:393` — a failed install is reported, and Not Now brings the ask back (Hooks consent gate › when settings.json cannot be parsed)
- `e2e/claude/hooks-on/consent.spec.ts:439` — clicking the office around the bubble neither answers nor dismisses (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:476` — a pre-consent 14-event install migrates to 12 with no prompt (Hooks consent gate / pre-consent install)
- `e2e/claude/hooks-on/consent.spec.ts:516` — Settings toggle removes the migrated hooks and keeps third-party entries (Hooks consent gate / pre-consent install)
- `e2e/claude/hooks-on/consent.spec.ts:564` — a failed uninstall does not persist hooks-off (Hooks consent gate / toggle-off failure)
...
- `e2e/standalone/hooks.spec.ts:12` — propagates hook-driven lifecycle into the browser UI (Standalone / hooks)
- `e2e/standalone/hooks.spec.ts:143` — the tokened page shows the Intro and Install writes the hooks (Standalone / hooks consent)
- `e2e/standalone/hooks.spec.ts:170` — an untokened spectator page never sees the consent dialog (Standalone / hooks consent)
- `e2e/standalone/hooks.spec.ts:199` — the hooks checkbox reflects install state and its click is the consent grant (Standalone / hooks consent)
