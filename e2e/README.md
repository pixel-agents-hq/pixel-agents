...
### Task board (`@area:tasks`)

The per-agent task board beside the office, fed by the task list an agent maintains
itself (Claude: TodoWrite, forwarded whole on every PreToolUse). Covers the board
appearing on the first list, grouping rows in_progress -> pending -> completed with
the agent's own order preserved inside a status, replacing the list wholesale on the
next revision, retracting on an empty list, the Tasks toggle hiding and reshowing it,
and /clear emptying a board that described the session that just ended. Hook-driven
only: the heuristic JSONL path publishes no task list, so there is no hooks-off
variant.
...
101 tests total. Generated by `scripts/generate-e2e-inventory.mjs`. Re-run after adding or removing tests.
...
- `e2e/standalone/taskboard.spec.ts:25` — the board survives a page reload and the replay does not send the agent walking (Standalone / task board)
- `e2e/standalone/taskboard.spec.ts:87` — an empty list is not replayed, so a retracted board stays retracted (Standalone / task board)
- `e2e/standalone/taskboard.spec.ts:125` — a task title with no spaces wraps instead of scrolling the board sideways (Standalone / task board)
...
- `e2e/claude/hooks-on/taskboard-selection.spec.ts:35` — an agent walks to the office task board when it revises its list, then returns (Hooks ON / task board — office errand and selection)
- `e2e/claude/hooks-on/taskboard-selection.spec.ts:76` — a revision inside the cooldown updates the board without a second walk (Hooks ON / task board — office errand and selection)
- `e2e/claude/hooks-on/taskboard-selection.spec.ts:130` — the tab strip switches boards and selects that agent in the office (Hooks ON / task board — office errand and selection)
- `e2e/claude/hooks-on/taskboard-selection.spec.ts:199` — selecting an agent switches the board; one with no list hides it but keeps the tabs (Hooks ON / task board — office errand and selection)
- `e2e/claude/hooks-on/taskboard.spec.ts:38` — publishes a task board from TodoWrite, replaces it wholesale, and retracts it on an empty list (Hooks ON / task board)
- `e2e/claude/hooks-on/taskboard.spec.ts:113` — the Tasks toggle hides and reshows the board (Hooks ON / task board)
- `e2e/claude/hooks-on/taskboard.spec.ts:157` — /clear empties the board while the character is reassigned (Hooks ON / task board)
