...
100 tests total. Generated by `scripts/generate-e2e-inventory.mjs`. Re-run after adding or removing tests.
...
- `e2e/standalone/taskboard.spec.ts:22` — the board survives a page reload and the replay does not send the agent walking (Standalone / task board)
- `e2e/standalone/taskboard.spec.ts:84` — an empty list is not replayed, so a retracted board stays retracted (Standalone / task board)
